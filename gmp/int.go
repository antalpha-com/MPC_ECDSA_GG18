...
/*
//#cgo CFLAGS: -ICPP/include
//#cgo LDFLAGS: -LCPP/lib -lgmp
...
func (z *Int) UnmarshalInt(a *IntCode) *Int {
	z.SetBytes(a.Data)
	sign := a.Sign
	if sign == 1 {
		z.SetSign(-1)
...
//	 0 if x == 0
//	+1 if x >  0
func (z *Int) SetSign(sign int) *Int {
	z.doinit()
	tmpsign := int(C._mpz_sgn(&z.i[0]))
	if tmpsign == sign {
		return z
	} else {
		return z.Neg(z)
...
func (n *Int) IntToBigInt() *big.Int {
	nBytes := n.Bytes()
...
func BigIntToInt(n *big.Int) *Int {
	nBytes := n.Bytes()
	nInt := new(Int).SetBytes(nBytes)
	if n.Sign() == -1 {
		nInt.SetSign(-1)
	}
	return nInt
...
func (z *Int) Bits() []Word {
	...
	w := make([]Word, 1+(z.BitLen()+31)/32)
	n := C.size_t(len(w))
	C.mpz_export(unsafe.Pointer(&w[0]), &n, -1, 8, 0, 0, &z.i[0])
	return w[0:n]
}
...
func (z *Int) SetBits(abs []Word) *Int {
	...
	if len(abs) == 0 {
		...
		C.mpz_import(&z.i[0], C.size_t(i), -1, 8, 0, 0, unsafe.Pointer(&abs[0]))
		// z.init = false
	}
	return z
}
...
func (z *Int) ModAdd(x, y, n *Int) *Int {
	z.Add(x, y)
	z.Mod(z, n)
	return z
}
...
func (z *Int) ModSub(x, y, n *Int) *Int {
	z.Sub(x, y)
	z.Mod(z, n)
	return z
}
...
// Mul sets z to the product x*y and returns z.
func (z *Int) ModMul(x, y, n *Int) *Int {
	z.Mul(x, y)
	z.Mod(z, n)
	return z
}
...
// Mod sets z to the modulus x%n for n != 0 and returns z.
// If n == 0, a division-by-zero run-time panic occurs.
// Mod implements Euclidean modulus (unlike Go); see DivMod for more details.
func (z *Int) Mod(x, n *Int) *Int {
	x.doinit()
	n.doinit()
	z.doinit()
	switch n.Sign() {
	case 1:
		C.mpz_fdiv_r(&z.i[0], &x.i[0], &n.i[0])
	case -1:
		C.mpz_cdiv_r(&z.i[0], &x.i[0], &n.i[0])
	case 0:
		panic("Division by zero")
	}
	return z
...
// String returns the decimal representation of z.十进制
func (z *Int) String() string {
	return z.string(10)
}

// String returns the Hexadecimal representation of z.
func (z *Int) Hex() string {
	return strings.ToUpper(z.string(16))
}
...
func (z *Int) ExpI(x, y, m *Int) *Int {
	yAbs := new(Int).Abs(y)
	z.Exp(x, yAbs, m)
	inverted := new(Int).ModInverse(z, m)
	if y.Sign() <= 0 {
		z.Set(inverted)
	}
	return z
...
func Jacobi(a, b *Int) int {
	a.doinit()
	b.doinit()
	return int(C.mpz_jacobi(&a.i[0], &b.i[0]))
}
...
func (z *Int) RandomPrime(pbitlen uint) *Int {
	z.doinit()
	one := new(Int).SetUint64(1)
	key_p := new(Int).SetUint64(1)
	key_p.doinit()
	// 使用GMP自带的素数生成函数
	n := new(Int).SetUint64(1)
	n.Lsh(one, pbitlen)

	b := new(big.Int).SetInt64(1)
	one1 := new(big.Int).SetUint64(1)
	b.Lsh(one1, pbitlen)
	fmt.Printf("max Int is : %v\n", b)
	truerand, err := rand.Int(rand.Reader, b)
	if err != nil {
		fmt.Printf("Can't generate random value: %v", err)
		return z
	}
	// buf := make([]byte, (pbitlen+7)>>3)
	randbuf := truerand.Bytes()
	key_p.SetBytes(randbuf)

	fmt.Println(key_p.Hex())
	C.mpz_nextprime(&key_p.i[0], &key_p.i[0]) //
	z.i = key_p.i
	fmt.Println(z.Hex())
	fmt.Println(z.BitLen())
	return z
}
func (z *Int) RandomSafePrime(pbitlen uint) *Int {
	z.doinit()
	one := new(Int).SetUint64(1)
	key_p := new(Int).SetUint64(1)
	key_p.doinit()
	...
	for {
		C.mpz_nextprime(&key_p.i[0], &key_p.i[0]) //
		key_q := new(Int).SetUint64(0)
		// fmt.Println("key_p:", key_p.Hex())
		key_q.Rsh(key_p, 1)
		// fmt.Println("key_q:", key_q.Hex())
		if !key_q.ProbablyPrime(20) {
			// fmt.Printf("prime found to be non-prime ")
		} else {
			break
		}
	}
...
func RangeRand(min, max *Int) *Int {
	...
	// var bigmin,bigmax big.Int
	bigmin := min.IntToBigInt()
	bigmax := max.IntToBigInt()
	bigmax.Add(bigmax, new(big.Int).SetUint64(1))
	tmpInt := new(big.Int)
	tmpInt.Sub(bigmax, bigmin)
	result, _ := rand.Int(rand.Reader, tmpInt)
	tmpInt = tmpInt.Add(result, bigmin)
	buf := tmpInt.Bytes()
	out := new(Int).SetBytes(buf)
	return out
}

// Rand sets z to a pseudo-random number in [0, n) and returns z.
func (z *Int) Rand(rnd *pseudorand.Rand, n *Int) *Int {
	z.doinit()
	// Get rid of n <= 0 case
	if n.Sign() <= 0 {
		z.SetInt64(0)
...
func (z *Int) Sqrt(x *Int) *Int {
	...
	xBig := x.IntToBigInt()
	zBig := xBig.Sqrt(xBig)
	zInt := BigIntToInt(zBig)
	// fmt.Println(zInt.Hex())
	// mulcheck := new(Int).Mul(zInt, zInt)
	// if mulcheck.Cmp(x) != 0 {
	// 	panic("sqrt error")
	// }
	return zInt
...
//			break
//		}
//		if x.init || x.Cmp(p) >= 0 { // ensure 0 <= x < p
//			x = new(Int).Mod(x, p)
//		}
//
//		switch {
//		case new(Int).Mod(p, _Int4) == _Int3:
//			// Check whether p is 3 mod 4, and if so, use the faster algorithm.
//			return z.modSqrt3Mod4Prime(x, p)
//		case new(Int).Mod(p, _Int8) == _Int5:
...
//		}
//		// x[i] != 0
//		return count*bits.UintSize + uint(bits.TrailingZeros(uint(bytes[count])))
//	}
type IntGmpCode struct {
	Sign uint32
	Data []byte
}

// MarshalNat Converts Nat Type to Natcode Type
func (z *Int) MarshalGmp() *IntGmpCode {
	// TODO(gri): get rid of the []byte/string conversions
	z.doinit()
	m := new(IntGmpCode)
	sign := z.Sign()
...
// UnmarshalNat Converts Natcode Type to Nat Type
func (z *Int) UnmarshalGmp(a *IntGmpCode) *Int {
	z.doinit()
	z.SetBytes(a.Data)
	sign := a.Sign
	if sign == 1 {

...
type (
	...
	ProofCode [ProofIters]*big.IntCode
...
// GenerateKeyPair generates a Paillier Key Pair, returning PrivateKey and PublicKey
...
// EncryptAndReturnRandomness enc and returns the Randomness
...
// HomoAdd sets c to the homomorphic sum c1 ⊕ c2.
// c ← c1•c2 (mod N²).
...
// QSquare returns Q*Q
func (privatekey *PrivateKey) QSquare() *big.Int {
	return new(big.Int).Mul(privatekey.Q, privatekey.Q)
}

// PSquare returns P*P
func (privatekey *PrivateKey) PSquare() *big.Int {
	return new(big.Int).Mul(privatekey.P, privatekey.P)
}
...
func (privateKey *PrivateKey) Decrypt(c *big.Int) (m *big.Int, err error) {
	...
	Lc := L(privateKey.CRTExpN2(c, privateKey.LambdaN), privateKey.N)
	// 2. L(u) = (Gamma^LambdaN-1 mod N2) / N
	// Lg := L(new(big.Int).Exp(privateKey.Gamma(), privateKey.LambdaN, N2), privateKey.N)
	Lg := L(privateKey.CRTExpN2(privateKey.Gamma(), privateKey.LambdaN), privateKey.N)
	// 3. (1) * modInv(2) mod N
...
func (sk *PrivateKey) CRTExpN2(m *big.Int, e *big.Int) *big.Int {
	c1 := new(big.Int).SetUint64(0)
	pinvsquared := new(big.Int).ModInverse(sk.PSquare(), sk.QSquare())
	// func (z *Nat) CRTExpN2(x, e, n2, p2, q2, p, q, pinv2 *Nat) *Nat {
	CRTExpN2(c1, m, e, sk.NSquare(), sk.PSquare(), sk.QSquare(), sk.P, sk.Q, pinvsquared)
	return c1
}

func CRTExpN2(z, x, e, n2, p2, q2, p, q, pinv2 *big.Int) *big.Int {

	xp := new(big.Int).SetUint64(0)
	xq := new(big.Int).SetUint64(0)

	ep := new(big.Int).SetUint64(0)
	eq := new(big.Int).SetUint64(0)

	// one := new(Nat).SetUint64(1)
	ps1 := new(big.Int).SetUint64(0)
	qs1 := new(big.Int).SetUint64(0)
...
func CodeToProof(p *ProofCode) *Proof {
	z := new(Proof)
	for i := 0; i < ProofIters; i++ {
		z[i] = new(big.Int).UnmarshalInt(p[i])
	}
	return z
}

func (privateKey *PrivateKey) ProofMal(k *big.Int, ecdsaPub *crypto2.ECPoint) Proofbuf {
	proof := privateKey.Proof(k, ecdsaPub)
	proofcode := ProofToCode(proof)
	buf, _ := cbor.Marshal(proofcode)
	var proofbuf Proofbuf
	// proofbuf := new(Proofbuf)
	proofbuf.Malbuf = buf
	return proofbuf
}

func (pf Proofbuf) VerifyMal(pkN, k *big.Int, ecdsaPub *crypto2.ECPoint) (bool, error) {
	proofcode := &ProofCode{}
	cbor.Unmarshal(pf.Malbuf, proofcode)
	proof := CodeToProof(proofcode)
	return proof.Verify(pkN, k, ecdsaPub)
}

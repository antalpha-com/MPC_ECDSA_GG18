...
// Start function in the round2 struct is the entry point for the second round of the protocol
// it represents the operation that round2 needs to perform
// a.verify dln proofs and record the opponent
// b. Synchronously wait until all received messages are validated
// c. Save parameters
// d. Send shares (polynomial values brought into different participants) to other participants point-to-point
// e. Broadcast the de-commitment of Shamir poly*G
// f.The update part verifies whether the message is legal
func (round *round2) Start() *tss.Error {
	...
	// create two slices dlnProof1FailCulprits and dlnProof2FailCulprits of length len(round.temp.kgRound1Messages) to store the culprits where DLN proofs fail.
	...
	// create a sync.WaitGroup instance wg to wait for concurrent operations to complete.
	wg := new(sync.WaitGroup)
	// iterate over each element in the round.temp.kgRound1Messages slice
	for j, msg := range round.temp.kgRound1Messages {
		// parse the 'r1msg' from the message, which is the content of type KGRound1Message.
		...
		// check if the bit length of paillierPKj's modulus meets the requirements
		...
		// check if H1j and H2j are equal, and returns an error if they are.
		if H1j.Cmp(H2j) == 0 {
			return round.WrapError(errors.New("h1j and h2j were equal for this party"), msg.GetFrom())
		}
		// check if the bit length of NTildej meets the requirements
		...
		// convert H1j and H2j to hexadecimal string representations and stores them in h1H2Map. If a duplicate key already exists, it returns an error.

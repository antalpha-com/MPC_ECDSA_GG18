...
func (round *round2) Start() *tss.Error {
	...
	for j, Pj := range round.Parties().IDs() {
		...
		go func(j int, Pj *tss.PartyID) {
			...
			// Verify(Session, ec, pkA, NTildeA, h1A, h2A, cA, cB, B)
			if i == 1 || Pj.Index == 1 {
				pi2ji.Verify(ContextI, round.Parameters.EC(), round.key.PaillierPKs[j], round.key.NTildej[j], round.key.H1j[j], round.key.H2j[j], r1msg.UnmarshalC(), c2ji, round.temp.bigWs[i])
			}
			// pi2ji.Verify(ContextI, round.Parameters.EC(), round.key.PaillierPKs[j], round.key.NTildej[j], round.key.H1j[j], round.key.H2j[j], r1msg.UnmarshalC(), c2ji, round.temp.bigWs[i])
		}(j, Pj)
	}

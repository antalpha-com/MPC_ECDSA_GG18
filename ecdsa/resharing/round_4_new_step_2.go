...
func (round *round4) Start() *tss.Error {
	...
	for j, msg := range round.temp.dgRound2Message1s {
		...
		go func(j int, msg tss.ParsedMessage, r2msg1 *DGRound2Message1) {
			defer wg.Done()
			// proofmod := r2msg1.ModProof
			// 将[][]byte转化为proofmod
			modProof, err := r2msg1.UnmarshalModProof()
			if err != nil {
				common.Logger.Warningf(" UnmarshalModProof failed for party %s", msg.GetFrom(), err)
			}
			ContextJ := common.AppendBigIntToBytesSlice(round.temp.ssid, big.NewInt(int64(j)))
			if ok := modProof.Verify(ContextJ, paiPK.N); !ok {
				// if ok := proofbuf.VerifyMal(ContextJ, paiPK.N); !ok {
				paiProofCulprits[j] = msg.GetFrom()
				common.Logger.Warning("modProof verify failed for party %s", msg.GetFrom())
			}
		}(j, msg, r2msg1)
		_j := j
		_msg := msg

...
// FirstRound creates and returns the first round of the re-sharing protocol
func (p *LocalParty) FirstRound() tss.Round {
	return newRound1(p.params, &p.input, &p.save, &p.temp, p.out, p.end)
}

// Start method is called to start the re-sharing protocol for the local party.
func (p *LocalParty) Start() *tss.Error {
	return tss.BaseStart(p, TaskName)
}

// Update method is to update the state of the local party upon receiving an incoming message.
